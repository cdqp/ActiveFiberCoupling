...
def run(ser, file):
    ...
    #will try to map one cross section's data onto the gaussian function
    for j in range(5):  #the loop will try to do it three times for each of the cross section but the code would change slightly because I didn't know how that was handled before
        waists.append(fittingvtwo.run(wavelength,x_voltages,z_voltages,powers)[3])
        if j != 4:
            move('y', ypos[j+1], ser)
            x_voltages, z_voltages, powers = scan(ser, step, file)
